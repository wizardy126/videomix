...
### 11.6 Interfaz de los bloques (T57)

- **Selección** (`useMixOverlays`): varios overlays sueltos y bloques (`selectedIds`, en orden de clic; `selectedLooseOverlayIds` y `selectedBlockIds` en orden de capas, que usa T58 para "Exportar selección"), o un **miembro editado dentro de su bloque** (`selectedMember`). Clic = solo ese; **Ctrl/Cmd+clic** lo quita o lo pone; **Mayús+clic** lo añade (`blocks/blockUi.ts`: `getSelectionMode`, `getNextSelection`). Funciona en los carriles y en las cajas de la mini vista; un miembro con modificador cuenta como su bloque.
- **Panel** (`OverlaySelectionPanel`, en el sitio de la lista de clips): un overlay suelto → `OverlayPanel`; un bloque → `BlockPanel`; un miembro → `OverlayPanel` en modo miembro (edita la definición con `updateBlockMember`, aviso "cambia sus N copias" si está enlazado, ancla "tiempo en el bloque" y destinos entre los miembros); varios → "Agrupar en bloque" (solo si todo son sueltos) y "Borrar" (un paso).
- **Carril "Bloques"** (debajo de los de overlays, `layoutBlockLane`): cada instancia es una pieza (color de su definición) desde su inicio hasta el fin de su contenido (sonidos incluidos) y, si no está plegada, sus miembros en sub-filas debajo; las piezas que se solapan van a filas distintas. La pieza **se arrastra entera** (`updateBlock` con el ancla desplazada, `getBlockMoveAnchor`; transitorio + un paso al soltar); el triángulo pliega/despliega (`collapsed`). Los miembros se seleccionan, arrastran y redimensionan como overlays (tiempos relativos, cambian todas las copias enlazadas). Iconos: bloqueado, oculto (pieza rayada), enlazado, aviso (tiempos, miembros o ficheros que faltan).
- **Bloqueado** (H8): la UI impide moverlo (carril y mini vista), editarlo, borrarlo, desagruparlo, repetirlo o cambiar su capa; su **contenido** (miembros, nombre, color, duración) no se edita mientras **cualquier** copia enlazada esté bloqueada (`isBlockDefLocked`); "Desvincular" una copia no bloqueada es la salida. Los `user*` de `useMixOverlays` lo comprueban además de los controles desactivados.
- **Oculto**: no se puede desagrupar (botón desactivado y aviso): sus overlays aparecerían.
- **Repetir…** (`BlockDialogs.askForBlockRepeat`): "N veces cada X s" (el bloque es la primera; N − 1 copias) o "al inicio de cada clip seleccionado" (los clips seleccionados en orden del vídeo, sin el clip en cuyo inicio ya está: `getRepeatClipIds`). **Duración del bloque…** (`askForBlockDuration`) → `stretchBlock`.
- **Anclas que conocen los bloques** (`getAnchorTargets`): overlays sueltos, bloques y miembros de cada instancia, quitando los que crearían **cualquier ciclo nuevo** (se aplica el cambio en una copia y se comparan los ciclos de `findOverlayCycleIds(expandBlocks(...).all)` y `composeBlockAnchors`).
- **Capas**: "Traer al frente / Enviar atrás" del panel del bloque (`moveBlockLayer`, entre bloques).
- **Proporción de salida**: al cambiarla, `updateSettings` reajusta también las imágenes de los miembros (`memberImageSizes`, por definición y miembro).
- **Ficheros que faltan de miembros**: `useMixWorkspace` los conserva (con `blockDefId`), el panel del bloque ofrece "Localizar..." (`updateBlockMember`) y la pieza muestra el aviso.

### 11.7 Plantillas: exportar, importar y biblioteca (T58)

- **Ficheros** (`blocks/blockTemplateFiles.ts`, puro con `fs`/`path` inyectados): `writeVmxBlockFile` escribe el `.vmxblock` de `getBlockExport` / `getOverlaysExport`. Sin "Incluir ficheros", las rutas quedan relativas al `.vmxblock` (o absolutas); con ella, cada fichero distinto se copia una vez a la carpeta **`<nombre>_files`** junto al `.vmxblock` (nombres repetidos → `logo (2).png`) y se guarda como `<nombre>_files/logo.png`. Si falta un fichero a incluir no se escribe nada. `readVmxBlockFile` resuelve las rutas relativas al `.vmxblock`; `findMissingTemplateFiles` y `relinkTemplateFile` sirven para "Localizar..." antes de importar.
- **Exportar** (menú Proyecto → Plantillas de bloques → Exportar bloque...): la selección de la vista Mix (T57) debe ser **un bloque** o **solo overlays sueltos** (`getTemplateSelection`; varios bloques o mezclas no, porque no hay bloques anidados). Diálogo con nombre y "Incluir ficheros" (desmarcada por defecto), luego el diálogo de guardar. No se escribe `$schema` (la ruta del esquema depende de dónde esté el `.vmxblock`); el esquema está en [`vmxblock.schema.json`](vmxblock.schema.json).
- **Importar** (botón "Importar bloque…" de la vista Mix y menú): `parseVmxBlockFile`; si hay errores, un diálogo con cada campo y motivo y no se importa nada. Diálogo (`BlockTemplateDialogs.tsx`) con vista previa (fotograma central, lista de miembros con sus tiempos relativos, duración, proporción), **colocación** (tiempos originales por defecto; desplazar ± s; en el cursor; anclado a un clip inicio/fin + desfase, con el clip del mismo nombre que el `clipAnchor` preseleccionado si existe), **variables** (`{{nombre}}` con sus valores del fichero o por defecto), **Adaptar** a la proporción del proyecto (si es otra; marcado por defecto) e "Importar desagrupado". Los ficheros que faltan se listan con "Localizar..." y el botón Importar queda desactivado hasta localizarlos todos. Un solo paso de deshacer: `addBlocks` (o `batch` con `addBlocks` + `ungroupBlock`), con ids nuevos (`instantiateVmxBlock`).
- **Biblioteca** (H3): carpeta fija `userData/block-library` (`getBlockLibraryDir`). "Guardar bloque en la biblioteca..." guarda siempre **con sus ficheros** (así la biblioteca no depende de dónde estén los originales) con un nombre de fichero único (`getNewLibraryFilePath`). "Insertar bloque…" (botón y menú) lista los `.vmxblock` de la carpeta (`listBlockLibrary`; los no válidos se muestran con su error) con nombre, número de elementos, duración y miniatura, y abre el diálogo de importar con la colocación **en el cursor**. "Abrir carpeta de la biblioteca" (`shell.openPath`).
- **Miniatura y vista previa** (`blocks/blockPreview.ts`): el fotograma central (mitad de `getBlockDefDuration`) dibujado con el dibujo de overlays de la previsualización en vivo (`getPreviewOverlayOps` + `drawPreviewFrame`) sobre un fondo gris, a 720p de la proporción. Sin ffmpeg: instantáneo, sin vídeo y sin ficheros de caché; se genera al abrir el diálogo. Refleja las variables y "Adaptar".
- **Acciones de teclado** (sin tecla por defecto, se pueden asignar): `importBlock`, `exportBlock`, `saveBlockToLibrary`, `insertBlockFromLibrary`, `openBlockLibraryFolder`.
