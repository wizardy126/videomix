...
- **Caché de render** (T28, `render/renderCache.ts`, puro): ver [T28](execution/T28-v2-render-incremental.md).
  ...
  - Carpeta `.<nombre>.vmx.cache/` junto al proyecto (sin guardar: `userData/videomix-cache/<id de sesión>`), con `render/` y `preview-<W>x<H>/` separados (y `converted/<hash>/` con las conversiones de previsualización de fuentes no reproducibles, T42, que la poda no toca). Tras un render correcto, `pruneRenderCache` borra lo que no usó ese render en su carpeta y aplica el máximo global `renderCacheMaxBytes` (configStore, 5 GB por defecto, 0 = sin caché) por LRU. Menú Project → "Clear render cache".
