...
import type { BlackBarsDetection, LoudnessMeasurement, MixBlock, MixBlockDef, MixClip, MixClipRotation, MixMusicPlaylist, MixMusicTrack, MixOverlay, MixProject, MixSettings, MixSource } from './types';
import { getOutputSize } from './types';
import type { OverlayTimeRange } from './overlays/anchors';
import { detachProjectReferences, duplicateBlock, groupOverlaysIntoBlock, moveBlockLayer, removeBlock, removeBlockMember, repeatBlock, stretchBlockDef, ungroupBlock, unlinkBlock } from './blocks/blockOperations';
import type { BlockRepeat } from './blocks/blockOperations';
...
import { addRotation, getClipRotation, getSourceFrame, rotateClipRects, rotateSize } from './clipRotation';
import { normalizeKeyframes, rescaleKeyframes, rotateKeyframes } from './clipKeyframes';
 ...
 * v7 (T56): keyed by overlay id (loose and expanded block members) and by block id, see `resolveProjectTimes`
 * (overlayRemoval.ts), so blocks anchored to what's removed keep their start too.
 */
export type ResolvedOverlayTimesForRemoval = ReadonlyMap<string, OverlayTimeRange> | undefined;

/** H1 (T56): editable fields of a block instance. `undefined` removes an optional one. */
export type MixBlockPatch = Partial<Omit<MixBlock, 'id' | 'defId'>>;

/** H1 (T56): editable fields of a block definition (its members have their own actions). */
export type MixBlockDefPatch = Partial<Pick<MixBlockDef, 'name' | 'color'>>;
...
export type MixProjectAction =
  ...
  /**
   * A7 (T44): caches (or clears, `undefined`) the black bars detection of a source. A cache: dispatch it outside the
   * history like `setSourceMeta` (no undo step, not dirty), see `useMixProject().setSourceBlackBars`.
   */
  | { type: 'setSourceBlackBars', sourceId: string, blackBars: BlackBarsDetection | undefined }
  /** Inserted at `index`, or appended. */
  | { type: 'addClip', clip: MixClip, index?: number | undefined }
  /** A9 (T44): `patch.keyframes` is stored sorted by time, and an empty list as none (see `normalizeKeyframes`). */
  | { type: 'updateClip', clipId: string, patch: MixClipPatch }
  /**
   * E9 (T38d): turns the clip's picture to `rotation` (clockwise), turning its rects (and its keyframes, A9) with it so
   * they keep the same framing. Nothing happens while the size of its source is unknown (the rects can't be turned
   * without it).
   */
  ...
  // Blocks of overlays (H1, H5, H6, H8; v7, T56): see blocks/blockOperations.ts and 04-diseno §11
  /**
   * Groups loose overlays into a new block, keeping every time (see `groupOverlaysIntoBlock`). `resolved`: the times
   * before the edit (`resolveProjectTimes`); `useMixProject().dispatch` adds it when missing, like for removals.
   */
  | { type: 'groupOverlays', overlayIds: string[], blockId: string, defId: string, name: string, color: number, resolved?: ResolvedOverlayTimesForRemoval }
  /** The members become loose overlays with the same times (see `ungroupBlock`). */
  | { type: 'ungroupBlock', blockId: string }
  /**
   * Adds definitions and instances (import, insert from the library), in one step. Instances are inserted at `index`
   * among the blocks, or on top. Throws on an id already in the project.
   */
  | { type: 'addBlocks', defs: MixBlockDef[], blocks: MixBlock[], index?: number | undefined }
  /** Anchor (e.g. a drag), variables (H4), hidden/locked (H8), collapsed. */
  | { type: 'updateBlock', blockId: string, patch: MixBlockPatch }
  | { type: 'updateBlockDef', defId: string, patch: MixBlockDefPatch }
  /** Edits a member of a definition: every linked instance changes (H5). Like `updateOverlay`; anchors are block-relative. */
  | { type: 'updateBlockMember', defId: string, memberId: string, patch: MixOverlayPatch }
  /** See `removeBlockMember`. */
  | { type: 'removeBlockMember', defId: string, memberId: string, resolved?: ResolvedOverlayTimesForRemoval }
  /** Removes an instance (and its definition when unused); what's anchored to it becomes absolute (see `removeBlock`). */
  | { type: 'removeBlock', blockId: string, resolved?: ResolvedOverlayTimesForRemoval }
  /** An independent copy (new definition), right above the original. */
  | { type: 'duplicateBlock', blockId: string, newBlockId: string, newDefId: string, name?: string | undefined }
  /** H5: gives a linked instance its own copy of the definition. */
  | { type: 'unlinkBlock', blockId: string, newDefId: string }
  /** H5: linked copies, see `repeatBlock`. */
  | { type: 'repeatBlock', blockId: string, newBlockIds: string[], repeat: BlockRepeat }
  /** H6: scales the block's definition (so all its linked instances) to last `duration` s, see `stretchBlockDef`. */
  | { type: 'stretchBlock', blockId: string, duration: number }
  /** Among the blocks (all drawn above the loose overlays). */
  | { type: 'moveBlockLayer', blockId: string, to: OverlayLayerMove }
...
const clipOptionalKeys: (keyof MixClip)[] = ['minRect', 'pinTime', 'groupId', 'link', 'extendBeyondMax', 'rotation', 'keyframes'];
  ...
  switch (action.type) {
    ...
    case 'removeSource': {
      ...
      return {
        ...
        ...detachProjectReferences(project, { clipIds, times: action.resolved }),
      };
    }
    ...
    case 'relinkSource': {
      ...
      // E9: a turned clip's rects live in the turned frame, which changes the same way turned; A9: the keyframes with them
      const clips = project.clips.map((c) => {
        if (c.sourceId !== source.id) return c;
        const clipChange = rotateFrameChange(change, getClipRotation(c));
        return withoutUndefined({ ...c, ...rescaleClipRects(c, clipChange), keyframes: rescaleKeyframes(c.keyframes, clipChange) }, clipOptionalKeys);
      });
      return { ...project, sources, clips };
    }

    case 'setSourceBlackBars': {
      const index = project.sources.findIndex((s) => s.id === action.sourceId);
      const source = project.sources[index];
      if (source == null || isEqual(source.blackBars, action.blackBars)) return project;
      const sources = [...project.sources];
      sources[index] = withoutUndefined({ ...source, blackBars: action.blackBars }, ['blackBars']);
    ...
    case 'updateClip': {
      ...
      const patch = 'keyframes' in action.patch ? { ...action.patch, keyframes: normalizeKeyframes(action.patch.keyframes) } : action.patch;
      if (clip == null || !hasChanges(clip, patch)) return project;
      const clips = [...project.clips];
      clips[index] = withoutUndefined({ ...clip, ...patch }, clipOptionalKeys);
      return { ...project, clips };
    }
    ...
    case 'rotateClip': {
      ...
      const from = getClipRotation(clip);
      const rects = rotateClipRects(clip, frame, from, action.rotation);
      const keyframes = rotateKeyframes(clip.keyframes, rotateSize(frame, from), addRotation(action.rotation, -from));
      // only a turn is stored (missing = 0)
      return mixProjectReducer(project, { type: 'updateClip', clipId: clip.id, patch: { ...rects, rotation: action.rotation !== 0 ? action.rotation : undefined, ...(keyframes != null && { keyframes }) } });
    }
    ...
    case 'removeClip': {
      ...
      return {
        ...
        clips,
        ...(prunedAlwaysVisible !== alwaysVisible.clipIds && { settings: { ...project.settings, alwaysVisible: { clipIds: prunedAlwaysVisible } } }),
        ...detachProjectReferences(project, { clipIds: new Set([action.clipId]), times: action.resolved }),
      };
    }
    ...
    case 'removeOverlay': {
      ...
      return { ...project, ...detachProjectReferences({ overlays, blocks: project.blocks }, { overlayIds: new Set([action.overlayId]), times: action.resolved }) };
    }
    ...
    case 'groupOverlays': {
      const { overlayIds, blockId, defId, name, color, resolved } = action;
      return groupOverlaysIntoBlock(project, { overlayIds, blockId, defId, name, color, times: resolved ?? new Map() });
    }

    case 'ungroupBlock': {
      return ungroupBlock(project, { blockId: action.blockId });
    }

    case 'addBlocks': {
      if (action.defs.length === 0 && action.blocks.length === 0) return project;
      const defIds = new Set(project.blockDefs.map((d) => d.id));
      action.defs.forEach(({ id }) => {
        if (defIds.has(id)) throw new Error(`Duplicate block definition id ${id}`);
        defIds.add(id);
      });
      const ids = new Set([...project.blocks.map((b) => b.id), ...project.overlays.map((o) => o.id)]);
      action.blocks.forEach(({ id }) => {
        if (ids.has(id)) throw new Error(`Duplicate block id ${id}`);
        ids.add(id);
      });
      const blocks = [...project.blocks];
      blocks.splice(action.index ?? blocks.length, 0, ...action.blocks);
      return { ...project, blockDefs: [...project.blockDefs, ...action.defs], blocks };
    }

    case 'updateBlock': {
      const index = project.blocks.findIndex((b) => b.id === action.blockId);
      const block = project.blocks[index];
      // flags are only stored when true (and variables when there are some)
      const patch: MixBlockPatch = { ...action.patch };
      (['hidden', 'locked', 'collapsed'] as const).forEach((key) => {
        if (key in patch && !patch[key]) patch[key] = undefined;
      });
      if ('variables' in patch && patch.variables != null && Object.keys(patch.variables).length === 0) patch.variables = undefined;
      if (block == null || !hasChanges(block, patch)) return project;
      const blocks = [...project.blocks];
      blocks[index] = withoutUndefined({ ...block, ...patch }, ['variables', 'hidden', 'locked', 'collapsed']);
      return { ...project, blocks };
    }

    case 'updateBlockDef': {
      const index = project.blockDefs.findIndex((d) => d.id === action.defId);
      const def = project.blockDefs[index];
      if (def == null || !hasChanges(def, action.patch)) return project;
      const blockDefs = [...project.blockDefs];
      blockDefs[index] = { ...def, ...action.patch };
      return { ...project, blockDefs };
    }

    case 'updateBlockMember': {
      const defIndex = project.blockDefs.findIndex((d) => d.id === action.defId);
      const def = project.blockDefs[defIndex];
      const index = def?.members.findIndex((m) => m.id === action.memberId) ?? -1;
      const member = def?.members[index];
      if (def == null || member == null || !hasChanges<Record<string, unknown>>(member, action.patch)) return project;
      const members = [...def.members];
      members[index] = withoutUndefined<Record<string, unknown>>({ ...member, ...action.patch }, ['font', 'shadow', 'linkedCountdownId']) as MixOverlay;
      const blockDefs = [...project.blockDefs];
      blockDefs[defIndex] = { ...def, members };
      return { ...project, blockDefs };
    }

    case 'removeBlockMember': {
      return removeBlockMember(project, { defId: action.defId, memberId: action.memberId, times: action.resolved });
    }

    case 'removeBlock': {
      return removeBlock(project, { blockId: action.blockId, times: action.resolved });
    }

    case 'duplicateBlock': {
      return duplicateBlock(project, action);
    }

    case 'unlinkBlock': {
      return unlinkBlock(project, action);
    }

    case 'repeatBlock': {
      return repeatBlock(project, action);
    }

    case 'stretchBlock': {
      const defId = project.blocks.find((b) => b.id === action.blockId)?.defId;
      const index = project.blockDefs.findIndex((d) => d.id === defId);
      const def = project.blockDefs[index];
      if (def == null) return project;
      const stretched = stretchBlockDef(def, action.duration);
      if (stretched === def) return project;
      const blockDefs = [...project.blockDefs];
      blockDefs[index] = stretched;
      return { ...project, blockDefs };
    }

    case 'moveBlockLayer': {
      const blocks = moveBlockLayer(project.blocks, action.blockId, action.to);
